...
export default function CheckIns() {
  ...
  // Compute this week's Monday in "YYYY-MM-DD"
  const thisWeekMonday = (() => {
    ...
    return tues.toISOString().slice(0, 10);
  })();

  // Do we already have a checkin for that date?
  const doneThisWeek = checkins.some(
    ci => new Date(ci.date).toISOString().slice(0, 10) === thisWeekMonday
  );
      ...
      <ul style={{ listStyle: "none", padding: 0 }}>
        {checkins.map(ci => (
          <li key={ci.id} className="modal checkins__modal">
            <h3>
              {new Date(ci.date).toLocaleDateString(undefined, {
                weekday: "long",
                month: "short",
                day: "numeric",
                year: "numeric"
            ...
            <Link className="button button-small button-outline" to={`/checkins/${ci.id}/edit`} style={{ marginLeft: 8 }}>
            ✎ Edit
          </Link>
          </li>
        ))}
